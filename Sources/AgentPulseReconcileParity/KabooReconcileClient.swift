...
/// 拉取 kaboo reconcile 的错误分类（脱敏，不含 URL / token / 正文）。
public enum ReconcileFetchError: Error, Equatable, Sendable {
    case invalidBaseURL
    case invalidPath
    /// reporting.json 的 authToken header 名必须是 kaboo 认可的 JWT header，否则打不通。
...
public struct KabooReconcileClient: Sendable {
    /// kaboo 组内 combinedAuth 仅当此 header 存在才触发 JWT 校验。
    ...
    /// 用配置 + base URL + 已取到的 token 构造并发送 GET，解码为 `KabooReconcileResponse`。
    /// token 只在 header 内使用，绝不进入日志。
    public func fetch(
        configuration: TokenReportingConfiguration,
        baseURL: URL,
        token: SecretToken
    ) async throws -> KabooReconcileResponse {
        ...
        do {
            return try JSONDecoder().decode(KabooReconcileResponse.self, from: response.body)
        } catch {
    ...
    /// content-type / content-encoding。auth header 名必须是 kaboo 认可的 JWT header。
    ...
    public static func makeRequest(
        ...
        // 只读 GET 到 kaboo reconcile 只依赖 JWT auth header 打通 combinedAuth；
