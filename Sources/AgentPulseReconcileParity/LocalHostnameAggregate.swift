...
/// 从本地账本 bucket / session 明细聚合出「与 kaboo per-hostname 口径对齐」的统计。
///
/// 纯值类型 + 纯静态函数，不含任何 I/O，因此离线可用 mock 数据完整验证聚合与口径逻辑。
///
/// 口径对齐要点（否则永远对不上）：
/// - **totalTokens 用 kaboo 口径**：每个 bucket 取 `max(reportedTotal, input+output+cached+reasoning)`，
///   **不含 cacheCreation**，再求和。本地 `UsageTokenCounts.total` 会把 cacheCreation 计入求和分支，
///   与 kaboo 不同，故这里单独实现，绝不复用 `counts.total`。
...
public struct LocalHostnameAggregate: Sendable, Equatable {
    ...
    /// kaboo 口径 total 之和（排除 cacheCreation）。
    public var totalTokensKabooBasis: Int64
    ...
    public init(
        ...
        self.totalTokensKabooBasis = totalTokensKabooBasis
    ...
    /// 单个 bucket 的 kaboo 口径 total：`max(reportedTotal, input+output+cachedInput+reasoningOutput)`，
    /// **排除 cacheCreationInput**，与 kaboo ingest GREATEST 语义逐字对齐。
    public static func kabooBucketTotal(_ counts: UsageTokenCounts) -> Int64 {
    ...
    public static func aggregate(
        ...
        for bucket in buckets {
            total = saturatedAdd(total, kabooBucketTotal(bucket.counts))
