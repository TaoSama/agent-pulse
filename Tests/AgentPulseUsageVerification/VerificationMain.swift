...
enum AgentPulseUsageVerification {
    ...
    /// 泛化来源分派：任意非 codex 来源都走 Claude-compatible 解析（按 message.id 归属 + overwrite）；
    ...
    private static func verifyGeneralizedSourceDispatch() throws {
        ...
        try require(seed.events[0].mergeStrategy == .overwrite, "non-codex event carries overwrite (message.id keyed, idempotent)")
        ...
        try require(anth.events[0].mergeStrategy == .overwrite, "arbitrary non-codex source carries overwrite (message.id keyed)")
