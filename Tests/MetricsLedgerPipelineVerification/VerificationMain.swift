...
private struct MetricsLedgerPipelineVerifier {
    ...
    func verifyRecoveredIncrementalBaselineClearsPendingWithoutFullRecompute() throws {
        ...
        try require(
            try ledger.recoverIncrementalBaselineIfSafe(hostname: host, currentParserVersion: UsageJSONLParser.parserVersion) == .recovered,
            "safe legacy ledger should recover an incremental baseline"
        )
        try withDatabase(database) { db in
    ...
    func verifyLargeBaselineRecoveryIsDeferred() throws {
        let database = try temporaryDatabaseURL()
        defer { cleanupDatabase(at: database) }
        let ledger = try UsageLedgerStore(path: database.path)
        let host = "host-a"
        ...
        let event = tokenEvent(id: "e1", source: "codex", session: "s1", file: "file-a", ts: ts, input: 10)
        try ledger.record(events: [event], checkpoint: completeCheckpoint("file-a", source: "codex", ts: ts), hostname: host)
        _ = try ledger.finalizeDerived(hostname: host)
        ...
        try require(recovery == .deferred, "large legacy ledger baseline recovery should defer instead of rebuilding")
        try withDatabase(database) { db in
            try require(try scalarInt(db, "SELECT COUNT(*) FROM sync_state WHERE key='raw_derivation_pending';") == 1, "deferred recovery must keep raw derivation pending")
